...
router.post('/bankoffer_submit',upload.single('filenames'), function(req, res, next) {
  try{
       pool.query("insert into bankandotheroffer (status, filenames, created_at, updated_at, user_admin) values($1,$2,$3,$4,$5)",[ req.body.status, req.file.filename, req.body.created_at, req.body.updated_at, req.body.user_admin],function(error,result){
              if(error)
